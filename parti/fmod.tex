...
\begin{divagazione}
  ...
  endomorfismi non banali (poiché hanno uguali campi) e ciò è
  impossibile poiché le cubiche con endomorfismi non banali sono in numero
  numerabile.
\end{divagazione}
...
Allora i tori complessi modulo isomorfismo sono in bigezione con $\cH$
...
\begin{proof}
  ...
  $gK \cap K \neq \emptyset$ per al più un numero finito di $g \in \Gamma$.
...
\zeta_3 = \zeta_3$: vedremo che sono gli unici due punti che danno fastidio.

\notamargine{Incollando i bordi del dominio fondamentale si vede
  che topologicamente lo spazio delle curve ellittiche è omeomorfo a
  $\bbC$. Non solo, a $F / \Gamma$ si può dare una naturale struttura complessa
  che lo rende biolomorfo a $\bbC$ tramite $j$. Noi non ce ne occuperemo direttamente.}
...
\begin{teorema}
  $\Gamma = \SL_2 \bbZ$ è generato da $S$ e da $T$. Inoltre è esattamente il
...
\begin{osservazione}
  ...
  (che, nei punti critici, non è quella data dall'immersione per via dei due stabilizzatori non banali),
  ...
  % ma possiamo fare ben meglio di cosi: le carte sono esplicite
  % TODO: scrivere e giustificare bene che la carta locale per un intorno di z è sum_{g in Stab(z)}{gz}.
\end{osservazione}
...
\begin{proof}
	...
	Poiché $f$ è una forma modulare, vale $f(Sz)=f(-\frac{1}{z})=z^{2k}f(z)$, da cui, prendendo la derivata logaritmica,
...
\begin{proof}
	...
	\begin{itemize}
		\item Per $k < 0$ vale $M_k = \{ 0 \}$, infatti per una forma modulare gli ordini sono tutti non negativi;
		
		\item Per $k = 0$ si ha $M_0 = \bbC$: infatti, per il teorema precedente, una forma modulare di peso $0$ è o nulla oppure non si annulla mai, quindi è olomorfa da una superficie di Riemann compatta e semplicemente connessa verso $\bbC$, quindi costante diversa da $0$;
		...
		\item Per $k = 2$ si ha $M_2=\bbC G_2$. Infatti le forme non nulle in $M_2$ si annullano solo in $\rho$ con ordine 1,
		...
		\item Per $k = 6$, si ha $M_6 = \bbC G_2^3 + \bbC G_3^2$. Vale infatti $G_2^3, G_3^2 \in M_6$, inoltre con combinazioni lineari di queste, possiamo trasformare una funzione $f \in M_6$ in un'altra $g$ con uno zero in $\rho$ o in $i$, dopodiché si applica lo stesso ragionamento dei punti precedenti.
		Le forme $G_2^3$ e $G_3^2$ sono linearmente indipendenti, per esempio perché si annullano in punti diversi, dunque sono una base di $M_6$.
		
		Notiamo inoltre che $\Delta \in M_6$, poiché combinazione lineare di $G_2^3$ e $G_3^2$.
		Poiché $\Delta(z) \neq 0$ in $\cH$, si ha $\Delta(\infty) = 0$ con ordine $1$.
	\end{itemize}
	
	Per $k > 6$ abbiamo che $G_k \in M_k$ e $G_k(\infty) \neq 0$.
...
L'invariante $j$ è una funzione modulare di peso $0$, dunque è invariante per trasformazioni in $\SL_2(\bbZ)$; ha un polo semplice in $\infty$ e nessun altro polo.

Sia ora $c \in \bbC$. Applicando il teorema \ref{170201-forme-ordini} alla funzione $j-c$, otteniamo che questa, avendo un solo polo, deve avere anche un solo zero (di ordine opportuno se in $\rho$ o in $i$). Dunque facendo variare $c$ si ottiene che la funzione $j$ assume ogni possibile valore complesso. Gioite! Per l'osservazione \ref{170123-j_suriettiva} abbiamo finalmente dimostrato che \emph{ogni cubica viene da un toro}.
\notamargine{In particolare $\quotient{\cH}{G} \overset{j}{\rar} \bbC$ è una bigezione.}

Aggiungiamo come nota finale che il campo delle funzioni modulari di peso $0$ è $\bbC(j)$. Ciò corrisponde al fatto che le funzioni meromorfe sulla sfera di Riemann sono $\bbC(x)$, letto attraverso il biolomorfismo $j : F \cup \{ \infty \} \rightarrow \hat \bbC$.

Per approfondire, Zannier consiglia la parte sulle funzioni modulari di ``A course in Arithmetic'', di Serre, o il libro di Zagier, ``The 1-2-3 of Modular Forms''.

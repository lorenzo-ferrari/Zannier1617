...
\begin{definizione}
    La funzione $\wp$ di Weierstrass è definita dalla serie
	\[
		\wp_L(z):=\frac{1}{z^2}+\sum_{\omega\in L\ \omega\not=0}\left(\frac{1}{(z-\omega)^2}-\frac{1}{\omega^2} \right).
	\]
    Inoltre la \emph{serie di Eisenstein di peso $k$} è data dalla serie
	\[
...
Sia $E_L$ il luogo di zeri dell'equazione $y^2=A_L(x)$ in $\bbC^2$ e $\tilde{E}_L$ il suo completamento proiettivo.
\begin{proposizione}

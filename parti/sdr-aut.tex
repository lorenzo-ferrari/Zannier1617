...
connesse, che il teorema di Riemann dice essere $\mathbb{P}^1(\mathbb{C})$, $\mathbb{C}$, $\cD$.
...
Quindi, essendo $G$ triplamente transitivo, possiamo assumere $\varphi( \infty ) = \infty$.
	Ora osserivamo che, essendo $\varphi$ un automorfismo olomorfo, detto $\cD=D(0,1)$, $\varphi (\cD)$ conterrà
...
\section{Automorfismi di $\cD$}
\begin{definizione}[Trasformazioni di Möbius]
	Si chiamano Trasformazioni di Möbius le trasformazioni $\cD \rar \cD$ del tipo $z\mapsto c \displaystyle{\frac{z-a}{1- \bar{a}z}}$ con $|c|=1$ e $a \in D$.
\end{definizione}
...
\begin{osservazione}
	Le trasformazioni di Möbius mandano $\cD$ in se stesso. Infatti:
...
\Longleftrightarrow \left( 1-|z|^2 \right) \left( 1-|a|^2 \right) >0$$
	Questa è vera $\forall z \in \cD$.
\end{osservazione}

\begin{osservazione}
	Il gruppo delle Trasformazioni di Möbius G ha dimensione topologica 3 (è come $S^1 \times \cD$),
...
\end{lemma}
\begin{proof}
	È evidente che si può mandare $0$ in qualsiasi punto di $\cD$ (anche ponendo $c=1$).
\end{proof}
...
\begin{lemma}[di Schwarz]
	Sia $f: \cD \rightarrow \cD$ una funzione olomorfa tale che $f(0)=0$.
	Allora $|f(z)| \leq |z|\ \ \forall z \in \cD$ e $|f'(0)| \leq 1$. Inoltre, se vale la prima uguaglianza anche solo in un punto, $f$ è una rotazione.
\end{lemma}
\begin{proof}
	$f(0)=0 \Rightarrow \frac{f(z)}{z}$ è olomorfa in $\cD$. Detto $D_r$ il disco di raggio $r$,
per il principio del massimo modulo si ha che, per $z \in D_r$,
	...
	\Longrightarrow \left| \frac{f(z)}{z} \right| \leq 1 \ \ \forall z \in \cD.
	\]
...
Infine, se si verifica un'uguaglianza, sempre per il principio del massimo modulo risulta che
	$\left| \frac{f(z)}{z} \right|$ è costante in $\cD$, quindi $f$ è una rotazione.
\end{proof}
...
\end{proposizione}
\begin{proof}
Sappiamo che $\Aut(\cD) \supseteq G$. Sia quindi $\varphi \in \Aut(\cD)$. Essendo $G$ transitivo posso supporre che $\varphi(0)=0$.
...
\subsection {$\mathcal{H}$, ovvero un altro modello per $\cD$.}

Vediamo ora che il disco $\cD$ è biolomorfo al semipiano $\mathcal{H} = \left \{ z \in \mathbb{C} | \ Im (z)>0 \right \}$
Presi $w \in \mathcal{H}$ e $z \in \cD$, consideriamo le mappe:
$$w \mapsto z=\frac{i-w}{i+w} \qquad z \mapsto w= i\frac{1-z}{1+z}$$
Si può verificare sono una l'inversa dell'altra e che sono effettivamente omeomorfismi tra $\mathcal{H}$ e $\cD$.
Quindi $\cD \simeq \mathcal{H}$ come superfici di Riemann.
...
$\hat{\bbC},\ \bbC, \ \cD. $
\end{teorema}
Sappiamo che dunque una generica superficie di Riemann $X$ avrà un rivestimento universale $\tilde{X}$, che dovrà essere uno dei tre precedenti e che $X$ può essere ricostruita quozientado $\sfrac{\tilde{X}}{G}$ dove $G<\Aut(\tilde{X})$ è un gruppo che agisce in maniera propriamente discontinua e senza punti fissi.\\
...
\end{definizione}
È opportuno osservare che le successioni esatte corte sono caratterizzate dalle proprietà:
...
\begin{proof}
    ...
    $$
    proiettando con $\pi$ su $W$ e usando l'ipotesi di lineare indipendenza sui $\{\gamma_i\}$ ottengo:
    $$
...
Possiamo ora usare che $S$ è iperbolica per mostrare il seguente importante
...
\notamargine{C'è anche una versione più forte di questo teorema: la stessa conclusione vale in ogni intorno di ogni singolarità essenziale.}
...
Allora $f$ induce una mappa olomorfa $f:\bbC \rightarrow S$ (con $S$ come in Esercizio 7).

...
$$
\omega=\frac{\de x}{y(x)}=\frac{\de x}{\pm\sqrt{ax^3+bx+c}}
$$
questa forma è definita su $\bbC\setminus\{\alpha, \beta, \gamma\}$ le tre radici della cubica ma ha un problema di scelta della determinazione della radice; perchè sia ben definita occorre operare dei {\it tagli} $\gamma_1$ e $\gamma_2$ che collegano le radici $\alpha,\beta$ e $\gamma,\infty$. Se rimuoviamo questi cammmini dal nostro piano una scelta del segno nella forma $\omega$ è coerente ovunque e si può felicemente integrare. Inoltre aver tolto l'$\infty$ ci permette di passare dal piano complesso alla sfera di Riemann.
% Da rivedere prima o poi: non capisco tutta la supercazzola sull'ambiguita' di y sopra. Ad occhio basta questo fatto:
Si noti che il pullback della di $\omega$ tramite la mappa di Weierstrass è proprio $\de z$.

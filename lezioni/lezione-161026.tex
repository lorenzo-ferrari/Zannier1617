...
\notamargine{Inoltre, potete mostrare che non vi sono mappe olomorfe da superfici sferiche a superfici iperboliche, utilizzando le carte su $\hat\bbC$ ed il teorema di Liouville su queste. Nell'altro verso invece ve ne sono di non costanti, in particolare potete prendere l'immersione da $D$ in $\bbC$ e quella da $\bbC$ in $\hat\bbC$}
	...
	\begin{proof}
		In due parole, la legge di gruppo su $T$ si solleva a $\bbC$, gli automorfismi di $\bbC$ sono solo le affinità e da questo concludo che l'unica legge di gruppo su $\bbC$, quindi su $T$, è quella additiva.

		In più di due parole, siano $x \in T$, $*$ una legge di gruppo e sia senza perdita di generalità $0$ l'elemento neutro. Consideriamo la funzione $\Phi: (x, z) \mapsto x * z - x$.

		Per $x$ fissato, questa è una mappa olomorfa tra tori, che sappiamo essere necessariamente della forma $z \mapsto c_x z + b_x$, con $c_x L \subseteq L$. Inoltre $\Phi(x, 0) = 0$, da cui $b_x = 0$. Sia allora $c_x \in \bbC$ tale che $\Phi(x, z) = c_x z$.
		Vogliamo dire che $c_x$ è olomorfa (in particolare continua) rispetto a $x$, localmente costante e quindi costante per connessione.
		Fissato un $\lambda \in L$ non nullo, la funzione $x \mapsto \lambda c_x$ assume valori nell'insieme discreto $L$, quindi per dire che $c_x$ è localmente costante basta la continuità.
		
		Chi conosce un po' di analisi complessa in più dimensioni direbbe semplicemente che $c_x = \partial_z \Phi$ è olomorfa, noi possiamo sollevare mappe.

		Solleviamo innanzitutto la moltiplicazione $\mu$ della struttura di gruppo su $T$ per ottenere una struttura di gruppo su $\bbC$. Dopodiché, fissato $0 \ne z_0 \in T$, sia $\tilde{z_0}$ una sua preimmagine. Le mappe su $\bbC$ definite da $\tilde{\mu}(\tilde{x}, \tilde{z_0}) - \tilde{x}$ e $c_x \tilde{z_0}$ sono entrambe sollevamenti di $x \mapsto x * z_0$, quindi $\tilde{\mu}(\tilde{x}, \tilde{z_0}) - \tilde{x} = c_x \tilde{z_0} + \lambda$ per un qualche $\lambda \in L$ costante.

...
(ii) Identico ad (i), usando la funzione $\frac{F'}{F}$ al posto di $F$. Infatti $\frac{F'}{F}$ è ancora biperiodica e $\res_x\frac{F'}{F}=\ord_xF$).
...
È inoltre facile verificare che $P$ è una funzione $L$-biperiodica.
...
Fattorizzando $P$, otteniamo una funzione meromorfa $\wp:T\rar\bbC$, chiamata \textbf{funzione di Weierstrass}. Attenzione: tale definizione dipende dal reticolo $L$ scelto per rappresentare $T$! Però non cambia in modo sostanziale: infatti la $\wp$ trasforma, per un cambio di reticolo $L \mapsto \alpha L$:
\[
...
Consideriamo ora la funzione $(\wp')^2$: è pari e ha un polo solo nell'origine, per cui $(\wp')^2 = A(\wp)$ per un opportuno $A \in \bbC[z]$. Grazie all'ordine di polo in zero, concludiamo $\deg A = 3$.

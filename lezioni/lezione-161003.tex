...
La richiesta di essere T2 non si deduce dalle altre condizioni. Infatti se consideriamo il ``disco con due origini'',
	cioè il quoziente $S/\!\!\sim$ dove $S=\mathbb{D} \times \{0\} \cup \mathbb{D} \times \{1\}$ e $x\sim y \Longleftrightarrow x=y$ oppure $x=(u,i),\ y=(u,j)$ con $\{i,j\}=\{0,1\}$,
...
connesse: $\mathbb{P}^1(\mathbb{C})$, $\mathbb{C}$, $\mathbb{D}$.
...
Quindi, essendo $G$ triplamente transitivo, possiamo assumere $\varphi( \infty ) = \infty$.
	Ora osserivamo che, essendo $\varphi$ un automorfismo olomorfo, detto $\mathbb{D} = D(0,1)$, $\varphi (\mathbb{D})$ conterrà
...
\section{Automorfismi di $\mathbb{D}$}
\begin{definizione}[Trasformazioni di Möbius]
	Si chiamano Trasformazioni di Möbius le trasformazioni $\mathbb{D} \rar \mathbb{D}$ del tipo $z\mapsto c \displaystyle{\frac{z-a}{1- \bar{a}z}}$ con $|c|=1$ e $a \in D$.
\end{definizione}
...
\begin{osservazione}
	Le trasformazioni di Möbius mandano $\mathbb{D}$ in se stesso. Infatti: 
...
\Longleftrightarrow \left( 1-|z|^2 \right) \left( 1-|a|^2 \right) >0$$
	Questa è vera $\forall z \in \mathbb{D}$.
\end{osservazione}

\begin{osservazione}
	Il gruppo delle Trasformazioni di Möbius $G$ ha dimensione topologica 3 (è come $S^1 \times \mathbb{D}$),
...
\end{lemma}
\begin{proof}
	È evidente che si può mandare $0$ in qualsiasi punto di $\mathbb{D}$ (anche ponendo $c=1$).
\end{proof}
...
\begin{lemma}[di Schwarz]
	Sia $f:\mathbb{D}\rightarrow \mathbb{D}$ una funzione olomorfa tale che $f(0)=0$. 
	Allora $|f(z)| \leq |z|\ \ \forall z \in \mathbb{D}$ e $|f'(0)| \leq 1$. Inoltre, se vale la prima uguaglianza anche solo in un punto, $f$ è una rotazione.
\end{lemma}
\begin{proof}
	$f(0)=0 \Rightarrow \frac{f(z)}{z}$ è olomorfa in $\mathbb{D}$. Detto $D_r$ il disco di raggio $r$,
...
Infine, se si verifica un'uguaglianza, sempre per il principio del massimo modulo risulta che
	$\left| \frac{f(z)}{z} \right|$ è costante in $\mathbb{D}$, quindi $f$ è una rotazione.
\end{proof}
...
\end{proposizione}
\begin{proof}
	Sappiamo che $\Aut(\mathbb{D}) \supseteq G$. Sia quindi $\varphi \in \Aut(\mathbb{D})$. Essendo $G$ transitivo posso supporre che $\varphi(0)=0$.
...
\subsection {$\mathbb{D} \simeq \mathcal{H}$}

Vediamo ora che il disco $\mathbb{D}$ è biolomorfo al semipiano $\mathcal{H} = \left \{ z \in \mathbb{C} | \ Im (z)>0 \right \}$
Presi $w \in \mathcal{H}$ e $z \in \mathbb{D}$, consideriamo le mappe:
$$w \mapsto z=\frac{i-w}{i+w} \qquad z \mapsto w= i\frac{1-z}{1+z}$$
Si può verificare sono una l'inversa dell'altra e che sono effettivamente omeomorfismi tra $\mathcal{H}$ e $\mathbb{D}$.
Quindi $\mathbb{D} \simeq \mathcal{H}$ come superfici di Riemann.
...
Ora quindi dovremo capire quali sottogruppi di $\Aut (\hat{\mathbb{C}})$, $\Aut(\mathbb{C})$ e $\Aut(\mathbb{D})$ agiscono in modo propriamente discontinuo

...
\item Si escluderanno i casi di Superficie di Riemann Sferica e Euclidea. $S$ non è compatta, quindi non può essere biolomorfa alla Sfera di Riemann e nemmeno ad un toro $\quotient{\bbC}{L}$ con $L$ reticolo di rango $2$. Rimangono da escludere i casi $\quotient{\bbC}{\bbZ}$ e $\bbC$.\\
...
\begin{teorema}[Piccolo teorema di Picard]
Ogni funzione olomorfa intera non costante assume tutti i valori tranne al più uno.
\end{teorema}

\begin{osservazione}
	Non può essere meglio di così: la mappa esponenziale assume tutti i valori tranne lo $0$. Notiamo inoltre che se la mappa $f$ come da ipotesi ha in più la proprietà di \emph{non} avere una discontinuità essenziale all'infinito, allora per Liouville si ha che è un polinomio, per cui il risultato segue dal Teorema Fondamentale dell'Algebra.
\end{osservazione}

\notamargine{C'è anche una versione più forte di questo teorema, cioè il Grande teorema di Picard: la tesi vale in ogni intorno di una singolarità essenziale.}
...
Allora $f$ induce una mappa olomorfa $f:\bbC \rightarrow S$ (con $S$ come in Esercizio 7).
	Si ha che quindi $f$ si solleva a $\tilde{f}:\bbC\rightarrow \mathbb{D}$, con $\mathbb{D}$ il disco aperto. DIAGRAMMA \\
